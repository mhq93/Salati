...
fun CalendarMonthView(
    ...
    fun hijriOf(date: LocalDate): HijrahDate = hijriChronology.date(date.plusDays(hijriOffsetDays.toLong()))
    ...
    Column {
        Row(
            ...
            IconButton(onClick = { onMonthChanged(-1) }) {
                Icon(
                    ...
                    tint = MaterialTheme.colorScheme.onSurface // <-- Replaced InkText
                )
            }
        ...
        Row(horizontalArrangement = Arrangement.SpaceBetween, modifier = Modifier.fillMaxWidth()) {
            listOf(DayOfWeek.SUNDAY, DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.FRIDAY, DayOfWeek.SATURDAY)
                .forEach { dayOfWeek ->
                    Text(
                        text = dayOfWeek.getDisplayName(TextStyle.SHORT, locale).replaceFirstChar { it.uppercase() },
