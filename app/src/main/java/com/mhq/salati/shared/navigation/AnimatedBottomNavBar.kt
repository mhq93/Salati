...
fun AnimatedBottomNavBar(
    ...
    // 1. EXTRACT COLORS BEFORE CANVAS
    val unselectedContentColor = MaterialTheme.colorScheme.secondary // <-- Replaced AccentEmerald
    val selectedContentColor = MaterialTheme.colorScheme.tertiary    // <-- Replaced AccentGold
    val barBackgroundColor = MaterialTheme.colorScheme.primaryContainer // <-- Replaced DarkGreenLight
    val bubbleIconColor = MaterialTheme.colorScheme.onTertiary       // <-- Replaced DarkGreenLight (icon on bubble)
    ...
    val targetCenterPx = horizontalEdgePaddingPx + (itemWidthPx * visualIndex) + (itemWidthPx / 2f)
    ...
    LaunchedEffect(selectedIndex) {
        ...
        bubbleScale.animateTo(1f, animationSpec = spring(dampingRatio = Spring.DampingRatioLowBouncy, stiffness = Spring.StiffnessMediumLow))
    }
    ...
    Box(
        ...
        Canvas(
            ...
            val path = Path().apply {
                ...
                cubicTo(cx - notchRadiusPx * 0.9f, 0f, cx - notchRadiusPx * 0.85f, notchRadiusPx * 1.15f, cx, notchRadiusPx * 1.15f)
                cubicTo(cx + notchRadiusPx * 0.85f, notchRadiusPx * 1.15f, cx + notchRadiusPx * 0.9f, 0f, notchEnd, 0f)
            ...
            drawPath(path = path, color = barBackgroundColor) // <-- Replaced DarkGreenLight
        }
        ...
        Box(
            ...
            modifier = Modifier
                .offset {
                    ...
                    IntOffset(x = centerFromStartPx - (bubbleSizePx / 2), y = bubbleOffsetYPx)
                }
                .size(bubbleSize)
                .graphicsLayer(scaleX = bubbleScale.value, scaleY = bubbleScale.value)
                .background(selectedContentColor, CircleShape) // <-- Replaced AccentGold
        ) {
            Icon(
                imageVector = items[selectedIndex].icon,
                contentDescription = items[selectedIndex].label,
                tint = bubbleIconColor, // <-- Replaced DarkGreenLight
        ...
        Row(
            ...
            items.forEachIndexed { index, item ->
                ...
                val iconAlpha by animateFloatAsState(if (isSelected) 0f else 0.7f, label = "iconAlpha")
                val iconScale by animateFloatAsState(if (isSelected) 0.5f else 1f, label = "iconScale")
                ...
                Column(
                    ...
                    if (!isSelected) {
                        Box(modifier = Modifier.size(28.dp), contentAlignment = Alignment.Center) {
                            Icon(
                                ...
                                tint = unselectedContentColor.copy(alpha = iconAlpha), // <-- Replaced AccentEmerald.copy
                    ...
                    Text(
                        text = item.label,
                        color = if (isSelected) selectedContentColor else unselectedContentColor.copy(alpha = 0.8f), // <-- Replaced AccentGold / AccentEmerald
                        fontSize = 14.sp,
...
fun AnimatedBottomNavBarPreview() {
    val mockItems = listOf(
        BottomNavItem("home", "Home", androidx.compose.material.icons.Icons.Filled.CheckCircle),
        BottomNavItem("settings", "Settings", androidx.compose.material.icons.Icons.Filled.CheckCircle)
    )
    SalatiTheme {
        AnimatedBottomNavBar(items = mockItems, selectedRoute = "home", onItemSelected = {})
    }
}
